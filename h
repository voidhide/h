...
do
    ...
    function Library:ChangeVisible(State)
        ...
        if type(Library.SyncWindowGlow) == "function" then
            pcall(Library.SyncWindowGlow)
        end
    end
    --
    ...
    function Library.SelfDestruct()
        ...
        Library.Keybind = {}
        Library.Ignores = {}
        if Library._UIGlowGui then
            pcall(function()
                Library._UIGlowGui:Destroy()
            end)
    ...
    function Library.Window(Title, Size)
        ...
        Window.MainOutline = WindowOutline
        Library.MainWindowOutline = WindowOutline
        do
            local glowGui = Instance.new("ScreenGui")
            glowGui.Name = "ArtefactUIGlow"
            glowGui.IgnoreGuiInset = true
            glowGui.ResetOnSpawn = false
            glowGui.DisplayOrder = -50
            glowGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
            pcall(function()
                glowGui.ClipToDeviceSafeArea = false
            end)
            ...
            glow.ZIndex = 0
            glow.Parent = glowGui
            local function parentGlow()
                pcall(function()
                    if gethui then
                        glowGui.Parent = gethui()
                    end
                end)
                if glowGui.Parent then
                    return
                end
                pcall(function()
                    if syn and syn.protect_gui then
                        syn.protect_gui(glowGui)
                    end
                end)
                pcall(function()
                    glowGui.Parent = game:GetService("CoreGui")
                end)
                if not glowGui.Parent then
                    pcall(function()
                        local lp = game:GetService("Players").LocalPlayer
                        glowGui.Parent = lp and lp:FindFirstChild("PlayerGui")
                    end)
                end
            end
            parentGlow()
            if Library.Flags.UIGlowEnabled == nil then
                Library.Flags.UIGlowEnabled = true
            end
            ...
            local function syncGlow()
                ...
                local enabled = Library.Flags.UIGlowEnabled ~= false and Library.WindowVisible ~= false
                glow.Visible = enabled
                glowGui.Enabled = enabled
                local intensity = tonumber(Library.Flags.UIGlowIntensity) or 70
                glow.ImageTransparency = 1 - (math.clamp(intensity, 0, 100) / 100) * 0.75
                local accent = (Library.Theme.Accent and Library.Theme.Accent[1]) or Color3.fromRGB(152, 188, 255)
                glow.ImageColor3 = accent
                local pos = WindowOutline.Position
                local size = WindowOutline.Size
                glow.Position = UDim2.fromOffset((pos.X or 0) - 22, (pos.Y or 0) - 22)
                glow.Size = UDim2.fromOffset((size.X or 0) + 44, (size.Y or 0) + 44)
            end
            Window.SyncGlow = syncGlow
            Library.SyncWindowGlow = syncGlow
            getgenv().__FridaySyncWindowGlow = syncGlow
            Library._UIGlowGui = glowGui
            Utility.AddConnection(RunService.RenderStepped, function()
                syncGlow()
            end)
                ...
                function Section:Toggle(Options)
                    ...
                    function Toggle:Keybind(Options)
                        local Keybind = {
                            ...
                            EnumType = ((Options.Key or Enum.KeyCode.Unknown).EnumType == Enum.KeyCode) and "KeyCode" or "UserInputType",
